  ...
  return (
    <Card className="relative overflow-hidden py-0 gap-0">
      ...
      <CardContent className="px-3 sm:px-6 pb-3 sm:pb-6">
        <div className={`text-lg sm:text-2xl font-bold ${valueColor} mb-1`}>
          {value}
        </div>
        <div className="flex items-center space-x-1">
          ...
          {(!change || change === "N/A") && changeDescription && (
            <span className="text-xs text-gray-500">{changeDescription}</span>
          )}
          {change && change !== "N/A" && (
            <>
