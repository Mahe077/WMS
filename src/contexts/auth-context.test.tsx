...
jest.mock("@/contexts/app-context", () => ({ useNotifications: () => ({ addNotification: mockAddNotification }) }))
...
function TestComponent() {
  ...
  return (
    <div>
      <span data-testid="isAuthenticated">{ctx.state.isAuthenticated ? "yes" : "no"}</span>
      <span data-testid="user">{ctx.state.user ? ctx.state.user.name : "none"}</span>
      <button onClick={() => ctx.login("test@example.com", "password")}>Login</button>
...
describe("AuthProvider", () => {
  ...
  it("initializes with user if valid token", async () => {
    ...
    ;(fetch as jest.Mock).mockResolvedValueOnce({
      ok: true,
      json: async () => ({ user: { id: "1", name: "Test", email: "t@e.com", role: "admin", permissions: ["read"] } }),
    })
    await act(async () => {
      render(
        <AuthProvider>
    ...
    expect(fetch).toHaveBeenCalledWith("/api/auth/validate", expect.anything())
  })

  it("login success", async () => {
    ;(fetch as jest.Mock).mockResolvedValueOnce({
      ok: true,
      json: async () => ({ user: { id: "1", name: "Test", email: "t@e.com", role: "admin", permissions: ["read"] }, token: "abc" }),
    })
    ...
    await waitFor(() => expect(getByTestId("isAuthenticated").textContent).toBe("yes"))
  ...
  //     </AuthProvider>
  //   )
  //   // Wait for the "Logout" button to appear (i.e., TestComponent rendered)
  //   await act(async () => {
  ...
  //   expect(localStorage.getItem("wms_token")).toBeNull()
  // })
  it("can() returns correct permission for admin", async () => {
    localStorage.setItem("wms_token", "valid-token");
    (fetch as jest.Mock).mockResolvedValueOnce({
      ok: true,
      json: async () => ({
    ...
    function PermTest() {
      ...
      return (
        <>
          ...
          <span data-testid="can-delete">{ctx.can("delete") ? "yes" : "no"}</span>
        </>
      );
    }
    ...
    expect(getByTestId("can-delete").textContent).toBe("yes");
  });

  it("can() returns correct permission for non non admin", async () => {
    localStorage.setItem("wms_token", "valid-token");
    (fetch as jest.Mock).mockResolvedValueOnce({
      ok: true,
      json: async () => ({
  ...
  it("hasRole() works for string and array", async () => {
    ...
    (fetch as jest.Mock).mockResolvedValueOnce({
      ok: true,
      json: async () => ({
        user: { id: "1", name: "Test", email: "t@e.com", role: "admin", permissions: [] },
        token: "valid-token"
      }),
    });
    function RoleTest() {
      ...
      return (
        <>
          <span data-testid="single">{ctx.hasRole("admin") ? "yes" : "no"}</span>
          <span data-testid="multi1">{ctx.hasRole(["admin", "user"]) ? "yes" : "no"}</span>
          <span data-testid="multi2">{ctx.hasRole(["user", "guest"]) ? "yes" : "no"}</span>
        </>
      );
    }
    const { getByTestId } = render(
  ...
  it("clearError resets error", async () => {
    ...
    ;(fetch as jest.Mock).mockResolvedValueOnce({
      ok: true,
      json: async () => ({ user: { id: "1", name: "Test", email: "t@e.com", role: "admin", permissions: [] }, token: "abc" }),
    })
    const { getByText } = render(
      <AuthProvider>
        <TestComponent />
  ...
  it("updateActivity updates lastActivity", async () => {
    ...
    function ActivityTest() {
      ctx = useContext(require("./auth-context").AuthContext) as AuthContextType
